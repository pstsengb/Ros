...
class MySub{
   ...
   public:
       MySub(ros::NodeHandle m_nh, ros::NodeHandle m_pnh);
...
MySub::MySub(ros::NodeHandle m_nh,ros::NodeHandle m_pnh){
   qq=false;
   ...
   nh_.param("bbb/width2", width, 4.0);
   ROS_INFO("%.2f",width);

   nh_.param("ccc/width2", width, 4.0);
   ROS_WARN("%.2f",width);
   
   pnh_.param("width2", width, 4.0);
   ROS_ERROR("%.2f",width);
}
