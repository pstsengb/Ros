...
typedef struct __3DPos_Data_  //ps add
{
	float X;
	float Y;
	float Z;
...
typedef struct __3DVel_Data_ //ps add
{
	float linear_v_X;
	float linear_v_Y;
	float linear_v_Z;
...
class turn_on_robot
{
	...
	private:
		...
		void Odom_Trans_Ref(float& m_a, uint8_t Data_High,uint8_t Data_Low);
		/* This node Publisher topic and tf */
		void Publish_Odom(tf2_ros::TransformBroadcaster& tf_br);//发表里程计
		void Publish_Pose();
		...
		Pos_Data Robot_3DPos; //ps add 
		Vel_Data Robot_3DVel; //ps add
		
        Vel_Pos_Data Robot_Vel;//机器人的速度
		Vel_Pos_Data Robot_Vel1;//ps add extra	
