...
sensor_msgs::Imu Mpu6050;//实例化IMU对象  // ps add Mpu6050_1 is angular vel
/**************************************
Date: May 31, 2020
...
void turn_on_robot::Odom_Trans_Ref(float& m_a, uint8_t Data_High,uint8_t Data_Low)
{
  float data_return;
  short transition_16;
      transition_16 = 0;
...
void turn_on_robot::Cmd_Vel_Callback(const geometry_msgs::Twist &twist_aux)
{
  ...
  ROS_INFO_THROTTLE(60.0,"New control command");//显示受到了新的控制指令  
  }
...
Function: 发布里程计相关信息
***************************************/
void turn_on_robot::Publish_Odom(tf2_ros::TransformBroadcaster& tf_br)
{
    tf2::Quaternion threeDQuaternion;
    threeDQuaternion.setRPY( Robot_3DPos.roll, Robot_3DPos.pitch, Robot_3DPos.yaw); 
    geometry_msgs::Quaternion odom_quat = tf2::toMsg(threeDQuaternion);
    nav_msgs::Odometry odom;//里程计话题消息数据类型
    odom.header.stamp = ros::Time::now();//当前时间 
    odom.header.frame_id = "odom";
    odom.pose.pose.position.x = Robot_3DPos.X;//位置
    odom.pose.pose.position.y = Robot_3DPos.Y;
    odom.pose.pose.position.z = Robot_3DPos.Z; 
    odom.pose.pose.orientation = odom_quat;
    //设置速度 
    odom.child_frame_id = robot_frame_id;
    odom.twist.twist.linear.x =  Robot_3DVel.linear_v_X;//X方向前进速度
    odom.twist.twist.linear.y =  Robot_3DVel.linear_v_Y;//y方向前进速度
    odom.twist.twist.angular.z = Robot_3DVel.angular_v_Z;  //角速度 
    //这个矩阵有两种，分机器人静止和动起来的时候用 这是扩展卡尔曼滤波的,官网提供的2个矩阵
    if(Robot_3DVel.linear_v_X == 0 && Robot_3DVel.angular_v_Z == 0)//如果velocity是零，说明编码器的误差会比较小，认为编码器数据更可靠
      memcpy(&odom.pose.covariance, odom_pose_covariance2, sizeof(odom_pose_covariance2)),memcpy(&odom.twist.covariance, odom_twist_covariance2, sizeof(odom_twist_covariance2));
    else//如果小车velocity非零，考虑到运动中编码器可能带来的滑动误差，认为imu的数据更可靠
      memcpy(&odom.pose.covariance, odom_pose_covariance, sizeof(odom_pose_covariance)),memcpy(&odom.twist.covariance, odom_twist_covariance, sizeof(odom_twist_covariance));       

    //Publish tf by tf2_ros
    ...
    if(ros::Time::now() - initial_time_<ros::Duration(5.0)){
      if(!transformStamped.transform.rotation.z==1){
        ROS_ERROR("Error: R %.2f, P %.2f,Y %.2f", Robot_3DPos.roll, Robot_3DPos.pitch, Robot_3DPos.yaw);
      }
    }
 ...
 if (Receive_Data.Frame_Header == FRAME_HEADER )//判断帧头
  {
    ...
    { 
      ...
      { 
        ...
        Odom_Trans_Ref(Robot_3DVel.linear_v_X, Receive_Data.rx[2],Receive_Data.rx[3]);
        //Robot_Vel.X = Odom_Trans(Receive_Data.rx[2],Receive_Data.rx[3]); //获取底盘X方向速度
        Robot_3DVel.linear_v_Y = Odom_Trans(Receive_Data.rx[4],Receive_Data.rx[5]); //获取底盘Y方向速度//Y速度仅在全向移动机器人底盘有效
        Robot_3DVel.angular_v_Z = Odom_Trans(Receive_Data.rx[6],Receive_Data.rx[7]); //获取底盘Z方向速度   //angular vel
        ...
        if(fabs(Mpu6050_Data.gyros_x_data * GYROSCOPE_RATIO)<=0.05) // ps add
          Mpu6050.angular_velocity.x =  0.0;
        else
          Mpu6050.angular_velocity.x =  Mpu6050_Data.gyros_x_data * GYROSCOPE_RATIO;
          Robot_3DVel.angular_v_X = Mpu6050.angular_velocity.x;

        if(fabs(Mpu6050_Data.gyros_y_data * GYROSCOPE_RATIO)<=0.05)  // ps add
          Mpu6050.angular_velocity.y =  0.0;
        else
          Mpu6050.angular_velocity.y =  Mpu6050_Data.gyros_y_data * GYROSCOPE_RATIO;
          Robot_3DVel.angular_v_Y = Mpu6050.angular_velocity.y;

        if(fabs(Mpu6050_Data.gyros_z_data * GYROSCOPE_RATIO)<=0.05)
          Mpu6050.angular_velocity.z =  0.0;
        else
          Mpu6050.angular_velocity.z =  Mpu6050_Data.gyros_z_data * GYROSCOPE_RATIO;
          Robot_3DVel.angular_v_Z = Mpu6050.angular_velocity.z;

        //if(fabs(Robot_Vel.Z<0.05))
        //    Robot_Vel.Z = 0.0;
        //获取电池电压
        transition_16 = 0;
...
void turn_on_robot::Control()
{
  ...
  while(ros::ok())
  {
   ...
   //Sampling_time是采样时间，虽然下位机发送的数据频率是固定的，这里计算里程增量以ROS系统的时间更加可靠精确。
    ...
    {
      Robot_3DPos.X += ((Robot_3DVel.linear_v_X * cos(Robot_3DPos.pitch)) * cos(Robot_3DPos.yaw)) *  Sampling_Time;
      Robot_3DPos.Y += ((Robot_3DVel.linear_v_X * cos(Robot_3DPos.pitch)) * sin(Robot_3DPos.yaw)) * Sampling_Time;
      Robot_3DPos.Z += (Robot_3DVel.linear_v_X * sin(-Robot_3DPos.pitch)) * Sampling_Time;  //for test result ,sin add minus let rviz normal
      Robot_3DPos.roll += Robot_3DVel.angular_v_X * Sampling_Time; //ps add roll
      Robot_3DPos.pitch += Robot_3DVel.angular_v_Y * Sampling_Time; //ps add pitch
      Robot_3DPos.yaw += Robot_3DVel.angular_v_Z * Sampling_Time; //ps add  yaw


      ROS_WARN("roll:%.2f",Robot_3DPos.roll);
      ROS_WARN("pitch:%.2f",Robot_3DPos.pitch);
      ROS_WARN("yaw:%.2f",Robot_3DPos.yaw);
      ROS_ERROR("-----------------------");

      Quaternion_Solution(Mpu6050.angular_velocity.x, Mpu6050.angular_velocity.y, Mpu6050.angular_velocity.z,\
                Mpu6050.linear_acceleration.x, Mpu6050.linear_acceleration.y, Mpu6050.linear_acceleration.z);//四元数解算

      Publish_Odom(tf_br);        //发布里程计话题
      //Publish_Pose();
...
{
  memset(&Robot_3DPos, 0, sizeof(Robot_3DPos)); //ps add
  memset(&Robot_3DVel, 0, sizeof(Robot_3DVel)); //ps add
